...
class Notification extends Component
{
    ...
    public function render()
    {
        ...
        if($this->user->user_type == 1){
            ...
            ->whereNotIn('notifications.notification_type', [106, 105, 100, 102, 0, 1, 2, 3, 4, 5, 6, 8, 9, 10])
            ...
            $this->notifications = Notifications::join('user', 'user.id', '=', 'notifications.user_id')
            ->whereNotIn('notifications.notification_type', [103, 100, 102, 110, 106])
            // ->where('notifications.is_supply', 1)
            ->orderBy('notifications.created_at', 'DESC')
            ->select('user.*', 'notifications.*','notifications.created_at as timecreated')
            ->limit(10)
            ...
            ->whereNotIn('notifications.notification_type', [100, 103, 105, 110, 106])
            ...
            $this->notifications = Notifications::join('user', 'user.id', '=', 'notifications.user_id')
            ->where('user.department', Auth::user()->department)
            ->whereNotIn('notifications.notification_type', [102, 103, 105, 110, 106])
            // ->where('notifications.is_supply', 0)
            ->orderBy('notifications.created_at', 'DESC')
            ->select('user.*', 'notifications.*','notifications.created_at as timecreated')
            ->limit(10)
            ...
            $this->notifications = Notifications::join('user', 'user.id', '=', 'notifications.user_id')
            ->where('user.department', Auth::user()->department)
            ->whereNotIn('notifications.notification_type', [102, 103, 105, 110, 100])
            // ->where('notifications.is_supply', 0)
            ->orderBy('notifications.created_at', 'DESC')
            ->select('user.*', 'notifications.*','notifications.created_at as timecreated')
            ->limit(10)
